...
Processing of external packages/TPLs and fully TriBITS compatible external packages
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

The processing of external packages/TPLs is influenced by whether the external
package is a regular TriBITS TPL or is a fully TriBITS-compliant external
package.  Here, a **Fully TriBITS-Compliant External Package** has a
...
defined a TriBITS project which is upstream from a fully TriBITS compliant
external package will be uniquely defined by calling ``find_package()`` on the
most downstream fully TriBITS-compliant external package that depends on it.
...
fully TriBITS-compliant external packages (i.e. fully TriBITS-compliant
...
don't have a downstream fully TriBITS-compliant external package dependency
...
* ``<Package>_IS_FULLY_TRIBITS_COMPLIANT``: Set the ``TRUE`` if the package
...
An external package with ``<Package>_IS_FULLY_TRIBITS_COMPLIANT=TRUE`` **AND**
...
The variable ``<Package>_IS_FULLY_TRIBITS_COMPLIANT`` is set right when the
...
``<Package>_IS_FULLY_TRIBITS_COMPLIANT=TRUE`` set.  While all external
packages/TPLs listed in a `<repoDir>/TPLsList.cmake`_ file will have
``<Package>_IS_FULLY_TRIBITS_COMPLIANT=FALSE`` set (except for those tagged
with ``TRIBITS_PKG`` which will have
``<Package>_IS_FULLY_TRIBITS_COMPLIANT=FALSE`` set).

NOTE: When a TriBITS TPL (i.e. ``<Package>_IS_FULLY_TRIBITS_COMPLIANT=FALSE``)
...
packages/TPLs that have ``<Package>_IS_FULLY_TRIBITS_COMPLIANT=TRUE`` **OR**
``<Package>_PROCESSED_BY_DOWNSTREAM_TRIBITS_EXTERNAL_PACKAGE=TRUE``.  And we
only call ``find_package()`` for those fully TriBITS-complient external
packages that have ``<Package>_IS_FULLY_TRIBITS_COMPLIANT=TRUE`` **AND**
...
fully TriBITS-compliant external package which are all of those external
packages for which ``<Package>_IS_FULLY_TRIBITS_COMPLIANT=FALSE`` **AND**
``<Package>_PROCESSED_BY_DOWNSTREAM_TRIBITS_EXTERNAL_PACKAGE=FALSE``.
