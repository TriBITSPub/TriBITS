...
class SphinxRstGenerator:
  ...
  def combine_documentation(self, docs_dir: str, change_url_to_landing_page: bool = True,
      change_title_of_docs_main_page: bool = True, title: str = '' \
    ) -> None:
  ...
  def main(self):
    ...
    child_rst = set()
    for doc_name, sources in self.paths.items():
      includes = self.generate_rst(source_file=sources.get('src'),
        src_path=sources.get('src_path'),
    ...
    grand_child_rst_lst = \
      [gc_rst for gc_rst in grand_child_rst if gc_rst not in self.already_modified_files]
